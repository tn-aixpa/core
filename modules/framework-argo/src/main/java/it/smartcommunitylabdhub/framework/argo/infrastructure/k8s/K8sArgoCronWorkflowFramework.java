...
public class K8sArgoCronWorkflowFramework extends K8sBaseFramework<K8sArgoCronWorkflowRunnable, K8sCronWorkflowObject> {
    ...
    public K8sCronWorkflowObject build(K8sArgoCronWorkflowRunnable runnable) throws K8sArgoFrameworkException {
        ...
        try {
            CronWorkflow workflow = new CronWorkflow();
            ...
            workflow.setSpec(new CronWorkflowSpec());
    ...
    private void sanitize(CronWorkflow workflow) {
        ...
        if (workflow.getSpec().getWorkflowSpec().getTemplates() != null) {
            for (Template template : workflow.getSpec().getWorkflowSpec().getTemplates()) {
    ...
    private void appendTemplateDefaults(CronWorkflow workflow, K8sArgoCronWorkflowRunnable runnable) throws K8sFrameworkException {
        // Prepare environment variables for the Kubernetes job
        Template templateDefaults = workflow.getSpec().getWorkflowSpec().getTemplateDefaults();
    ...
    public V1PodSecurityContext buildPodSecurityContext(K8sArgoCronWorkflowRunnable runnable) throws K8sFrameworkException {
