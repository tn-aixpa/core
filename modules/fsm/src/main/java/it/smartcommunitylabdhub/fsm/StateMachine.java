...
public class StateMachine<S, E, C> {
          ...
          try {
            ...
            for (int i = 0; i < path.size() - 1; i++) {
              ...
              stateDefinition
                .getInternalLogic()
                .map(internalFunc ->
                  applyInternalFunc((contextStateValue, stateMachineValue) ->
                    internalFunc.applyLogic(
                      contextStateValue,
                      stateMachineValue
                    )
                  )
    ...
    if (errorState != null) {
      ...
      if (errorStateDefinition != null) {
        ...
        errorStateDefinition
          ...
          .map(internalFunc ->
            applyInternalFunc((contextStateValue, stateMachineValue) ->
              internalFunc.applyLogic(contextStateValue, stateMachineValue)
            )
          )
          // Optional.empty() because no input is provided
          .orElse(Optional.empty());
      ...
      throw new IllegalStateException(
        "Error state not set" + " : " + this.getUuid()
      );
    }
  }
  ...
  public static class Builder<S, E, C> {
     ...
     * @return This builder instance, allowing for method chaining.
     */
    public Builder<S, E, C> withErrorState(
      S errorState,
      State<S, E, C> stateDefinition
    ) {
     ...
     * @return This builder instance, allowing for method chaining.
     */
    public <T> Builder<S, E, C> withEventListener(
      E eventName,
      Consumer<C> listener
    ) {
     ...
     */
    public Builder<S, E, C> withEntryAction(
      S state,
      Consumer<Optional<C>> entryAction
    ) {
     ...
     */
    public Builder<S, E, C> withExitAction(
      S state,
      Consumer<Optional<C>> exitAction
    ) {
    ...
    public StateMachine<S, E, C> build() {
      StateMachine<S, E, C> stateMachine = new StateMachine<>(
        currentState,
        initialContext
      );
