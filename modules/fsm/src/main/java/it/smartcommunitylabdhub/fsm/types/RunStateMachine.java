...
public class RunStateMachine {
  ...
  public StateMachine<RunState, RunEvent, Map<String, Object>> create(
    ...
    StateMachine.Builder<RunState, RunEvent, Map<String, Object>> builder =
      new StateMachine.Builder<>(initialState, Optional.of(initialContext));
    ...
    State<RunState, RunEvent, Map<String, Object>> completedState =
      new State<>();
    State<RunState, RunEvent, Map<String, Object>> errorState = new State<>();

    createState.addTransaction(
      new Transaction<>(RunEvent.BUILD, RunState.READY, context -> true)
    );
    ...
    readyState.addTransaction(
      new Transaction<>(RunEvent.RUNNING, RunState.RUNNING, context -> true)
    );

    readyState.addTransaction(
      new Transaction<>(RunEvent.PENDING, RunState.READY, context -> true)
    );

    readyState.addTransaction(
      new Transaction<>(RunEvent.COMPLETED, RunState.COMPLETED, context -> true)
    );
