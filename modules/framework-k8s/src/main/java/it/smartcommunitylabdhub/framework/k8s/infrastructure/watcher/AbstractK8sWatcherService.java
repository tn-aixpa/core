...
public abstract class AbstractK8sWatcherService {
    ...
    private void watchPods(String label) {
        client.pods().inNamespace(namespace).withLabel(label).watch(new Watcher<Pod>() {
            @Override
            public void eventReceived(Action action, Pod pod) {
                String labels = pod.getMetadata().getLabels() != null
                        ? pod.getMetadata().getLabels().entrySet().stream()
                        .map(e -> e.getKey() + "=" + e.getValue())
                        .reduce((a, b) -> a + ", " + b)
                        .orElse("No Labels")
                        : "No Labels";

                log.trace("📡 Pod Event: [{}] - Pod: [{}] - Labels: [{}]", action, pod.getMetadata().getName(), labels);
    ...
    protected void debounceAndRefresh(String runnableId, Runnable refreshAction) {
        ...
        debounceMap.compute(runnableId, (key, lastExecutionTime) -> {
