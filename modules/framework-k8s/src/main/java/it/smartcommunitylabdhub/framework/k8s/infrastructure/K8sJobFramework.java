...
public class K8sJobFramework
  implements Framework<K8sJobRunnable>, InitializingBean {
  ...
  public void execute(K8sJobRunnable runnable) throws CoreException {
    ...
    List<V1EnvFromSource> envVarsFromSource =
      k8sBuilderHelper.getV1EnvFromSource();

    List<V1EnvVar> envVars = k8sBuilderHelper.getV1EnvVar();
    List<V1EnvVar> runEnvFromSource = k8sBuilderHelper.geEnvVarsFromSecrets(
      runnable.getSecrets()
    );
    ...
    runnable
      .getEnvs()
      .forEach(env ->
        envVars.add(new V1EnvVar().name(env.name()).value(env.value()))
      );
      ...
      resources.setRequests(
        k8sBuilderHelper.convertResources(
          runnable
            ...
            .collect(
              Collectors.toMap(
                CoreResource::resourceType,
                CoreResource::requests
              )
            )
        )
      );
      resources.setLimits(
        k8sBuilderHelper.convertResources(
          runnable
            ...
            .collect(
              Collectors.toMap(CoreResource::resourceType, CoreResource::limits)
            )
        )
      );
    }
    ...
    V1PodSpec podSpec = new V1PodSpec()
      ...
      .nodeSelector(
        Optional
          ...
          .stream()
          .collect(
            Collectors.toMap(CoreNodeSelector::key, CoreNodeSelector::value)
          )
      )
      .volumes(volumes)
      .restartPolicy("Never");
    ...
    try {
      ...
      log.info(
        "Job created: " +
        Objects.requireNonNull(createdJob.getMetadata()).getName()
      );
    } catch (Exception e) {
    ...
    StateMachine<RunState, RunEvent, Map<String, Object>> fsm =
      runStateMachine.create(
          ...
          fMachine -> {
            try {
              ...
              if (
                ...
                Objects.requireNonNull(v1JobStatus).getFailed() != null
              ) {
                // Job has failed delete job and pod
                deleteAssociatedPodAndJob(jName, namespace, runnable);
              } else if (
                v1JobStatus.getActive() != null && v1JobStatus.getActive() > 0
              ) {
                ...
                String v1JobStatusString =
                  JacksonMapper.CUSTOM_OBJECT_MAPPER.writeValueAsString(
                    v1JobStatus
                  );
    ...
    return List.of(
      Stream
        .concat(
          ...
          Arrays.stream(runnable.getArgs())
        )
        .toArray(String[]::new)
    );
  }
   ...
   * @param runnable the runnable Type in this case K8SJobRunnable
   */
  private void deleteAssociatedPodAndJob(
    String jobName,
    String namespace,
    K8sJobRunnable runnable
  ) {
