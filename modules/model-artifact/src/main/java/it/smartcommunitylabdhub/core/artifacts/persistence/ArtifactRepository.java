...
public interface ArtifactRepository
    ...
    @NonNull
    Page<ArtifactEntity> findAll(@NonNull Pageable pageable);
    ...
    @Query(
        ...
        "(SELECT a2.name, a2.project, MAX(a2.created) FROM ArtifactEntity a2 " +
        "WHERE a2.project = :project GROUP BY a2.name, a2.project) " +
        "ORDER BY a.created DESC"
    )
    ...
    @Query(
        ...
        "AND a.created = (SELECT MAX(a2.created) FROM ArtifactEntity a2 " +
        "WHERE a2.project = :project AND a2.name = :name)"
    )
