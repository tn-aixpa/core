...
public class ProjectServiceImpl
    ...
    return projectRepository
      ...
      .map(project -> {
        List<FunctionEntity> functions =
          functionRepository.findAllLatestFunctionsByProject(project.getName());
        List<ArtifactEntity> artifacts =
          artifactRepository.findAllLatestArtifactsByProject(project.getName());
        List<WorkflowEntity> workflows =
          workflowRepository.findAllLatestWorkflowsByProject(project.getName());
        List<DataItemEntity> dataItems =
          dataItemRepository.findAllLatestDataItemsByProject(project.getName());
        List<SecretEntity> secrets = secretRepository.findByProject(
          project.getName()
        );
  ...
  public Page<Project> getProjects(
    ...
    try {
      ...
      Specification<ProjectEntity> specification = createSpecification(
        filter,
        projectEntityFilter
      );

      Page<ProjectEntity> projectPage =
        this.projectRepository.findAll(specification, pageable);
      ...
      return new PageImpl<>(
        projectPage
          ...
          .map(project -> {
            List<FunctionEntity> functions =
              functionRepository.findAllLatestFunctionsByProject(
                project.getName()
              );
            List<ArtifactEntity> artifacts =
              artifactRepository.findAllLatestArtifactsByProject(
                project.getName()
              );
            List<WorkflowEntity> workflows =
              workflowRepository.findAllLatestWorkflowsByProject(
                project.getName()
              );
            List<DataItemEntity> dataItems =
              dataItemRepository.findAllLatestDataItemsByProject(
                project.getName()
              );
  ...
  public Project updateProject(Project projectDTO, String name) {
    return Optional
      ...
      .map(project -> {
        final ProjectEntity projectUpdated = projectEntityBuilder.update(
          project,
          projectDTO
        );
  ...
  public List<Function> getProjectFunctions(String name) {
    return Optional
      ...
      .flatMap(projectName -> {
        try {
          List<FunctionEntity> functions = functionRepository.findByProject(
            projectName
          );
          return Optional.of(
            functions
  ...
  public List<Artifact> getProjectArtifacts(String name) {
    return Optional
      ...
      .flatMap(projectName -> {
        try {
          List<ArtifactEntity> artifacts = artifactRepository.findByProject(
            projectName
          );
          return Optional.of(
            artifacts
  ...
  public List<Workflow> getProjectWorkflows(String name) {
    return Optional
      ...
      .flatMap(projectName -> {
        try {
          List<WorkflowEntity> workflows = workflowRepository.findByProject(
            projectName
          );
          return Optional.of(
            workflows
