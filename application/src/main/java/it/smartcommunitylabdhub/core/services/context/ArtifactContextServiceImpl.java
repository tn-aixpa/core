...
public class ArtifactContextServiceImpl
    ...
    try {
      ...
      if (!projectName.equals(artifactDTO.getProject())) {
        throw new CustomException(
          "Project Context and Artifact Project does not match",
          null
        );
      }
  ...
  @Override
  public Page<Artifact> getLatestByProjectName(
    ...
    try {
      ...
      Specification<ArtifactEntity> specification = createSpecification(
        filter,
  ...
  @Override
  public Artifact getByProjectAndArtifactAndUuid(
    String projectName,
    String artifactName,
    String uuid
  ) {
    try {
      ...
      return this.artifactRepository.findByProjectAndNameAndId(
          projectName,
          artifactName,
          uuid
        )
  ...
  @Override
  public Artifact getLatestByProjectNameAndArtifactName(
    String projectName,
    String artifactName
  ) {
    try {
      ...
      return this.artifactRepository.findLatestArtifactByProjectAndName(
          projectName,
          artifactName
        )
        .map(artifact -> artifactDTOBuilder.build(artifact, false))
  ...
  public Artifact createOrUpdateArtifact(
    ...
    try {
      ...
      ArtifactEntity artifact = Optional
        .ofNullable(artifactDTO.getId())
        .flatMap(id -> {
          Optional<ArtifactEntity> optionalArtifact =
          ...
          if (optionalArtifact.isPresent()) {
            ...
            return Optional.of(
              this.artifactRepository.saveAndFlush(artifactUpdated)
            );
          } else {
            // Build a new artifact and store it in the database
            ArtifactEntity newArtifact = artifactEntityBuilder.build(
              artifactDTO
            );
  ...
  @Override
  @Transactional
  public Boolean deleteSpecificArtifactVersion(
    String projectName,
    String artifactName,
    String uuid
  ) {
    try {
      if (
        this.artifactRepository.existsByProjectAndNameAndId(
            projectName,
            artifactName,
            uuid
          )
      ) {
        this.artifactRepository.deleteByProjectAndNameAndId(
            projectName,
            artifactName,
            uuid
          );
  ...
  @Override
  @Transactional
  public Boolean deleteAllArtifactVersions(
    String projectName,
    String artifactName
  ) {
    try {
      if (
        artifactRepository.existsByProjectAndName(projectName, artifactName)
      ) {
        this.artifactRepository.deleteByProjectAndName(
            projectName,
            artifactName
          );
        return true;
