...
public class ArtifactServiceImpl
  ...
  public Page<Artifact> getArtifacts(
    ...
    try {
      ...
      Page<ArtifactEntity> artifactPage =
        this.artifactRepository.findAll(specification, pageable);
  ...
  public Artifact createArtifact(Artifact artifactDTO) {
    if (
      ...
      artifactRepository.existsById(artifactDTO.getId())
    ) {
      throw new CoreException(
        "DuplicateArtifactId",
  ...
  public Artifact updateArtifact(Artifact artifactDTO, String uuid) {
    ...
    return artifactRepository
      ...
      .map(artifact -> {
        try {
          ArtifactEntity artifactUpdated = artifactEntityBuilder.update(
            artifact,
            artifactDTO
          );
