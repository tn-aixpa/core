...
@Service
@Transactional
public class RunContextServiceImpl
  extends ContextService<RunEntity, RunEntityFilter>
    ...
    try {
      ...
      if (!projectName.equals(runDTO.getProject())) {
        throw new CustomException(
          "Project Context and Run Project does not match",
          null
        );
      }
  ...
  @Override
  public Page<Run> getAllRunsByProjectName(
    ...
    try {
      ...
      Specification<RunEntity> specification = createSpecification(
        filter,
        runEntityFilter
      );
  ...
  @Override
  public Run getByProjectAndRunUuid(String projectName, String uuid) {
    try {
      // Check project context
      checkContext(projectName);
      ...
      return this.runRepository.findByProjectAndId(projectName, uuid)
        .map(run -> runDTOBuilder.build(run))
        .orElseThrow(() ->
          new CustomException(ErrorList.RUN_NOT_FOUND.getReason(), null)
        );
    } catch (CustomException e) {
      throw new CoreException(
        ErrorList.INTERNAL_SERVER_ERROR.getValue(),
  ...
  @Override
  public Run updateRun(String projectName, String uuid, Run runDTO) {
    try {
      // Check that project context is the same as the project passed to the
      // runDTO
      if (!projectName.equals(runDTO.getProject())) {
