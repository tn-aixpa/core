...
@Service
@Transactional
public class TaskContextServiceImpl extends ContextService<TaskEntity, TaskEntityFilter> implements TaskContextService {
    ...
    public Task createTask(String projectName, Task taskDTO) {
        try {
            ...
            if (!projectName.equals(taskDTO.getProject())) {
                throw new CustomException("Project Context and Task Project does not match", null);
            }
            ...
            // new one
            TaskEntity task = (TaskEntity) Optional
                .ofNullable(taskDTO.getId())
                .flatMap(id ->
                    taskRepository
                        .findById(id)
                        .map(a -> {
    ...
    @Override
    public Task getByProjectAndTaskUuid(String projectName, String uuid) {
        try {
            // Check project context
            checkContext(projectName);

            return this.taskRepository.findByProjectAndId(projectName, uuid)
    ...
    @Override
    public Task updateTask(String projectName, String uuid, Task taskDTO) {
        try {
            // Check that project context is the same as the project passed to the
            // taskDTO
            if (!projectName.equals(taskDTO.getProject())) {
            ...
            TaskEntity task =
                this.taskRepository.findById(taskDTO.getId())
                    .map(a -> // Update the existing task version
