...
@Validated
@Tag(
  name = "Task context API",
  description = "Endpoints related to tasks management in Context"
)
public class TaskContextController extends AbstractContextController {
  ...
  public ResponseEntity<Page<Task>> getLatestTasks(
    ...
    return ResponseEntity.ok(
      this.taskContextService.getAllTasksByProjectName(
          filter,
          project,
          pageable
        )
  ...
  public ResponseEntity<Task> getTaskByUuid(
    ...
    return ResponseEntity.ok(
      this.taskContextService.getByProjectAndTaskUuid(project, uuid)
    );
  }
  ...
  public ResponseEntity<Task> updateUpdateTask(
    ...
    return ResponseEntity.ok(
      this.taskContextService.updateTask(project, uuid, taskDTO)
    );
  }
  ...
  public ResponseEntity<Boolean> deleteSpecificTaskVersion(
    ...
    return ResponseEntity.ok(
      this.taskContextService.deleteSpecificTaskVersion(project, uuid)
    );
  }
}
