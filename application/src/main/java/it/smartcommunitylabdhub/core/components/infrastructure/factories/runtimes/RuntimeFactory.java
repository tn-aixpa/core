...
public class RuntimeFactory {
  ...
  public RuntimeFactory(
    ...
    runtimeMap =
      runtimes
        .stream()
        .collect(
          Collectors.toMap(this::getRuntimeFromAnnotation, Function.identity())
        );
  }
  ...
  private String getRuntimeFromAnnotation(
    ...
    if (runtimeClass.isAnnotationPresent(RuntimeComponent.class)) {
      RuntimeComponent annotation = runtimeClass.getAnnotation(
   ...
   */
  public Runtime<? extends FunctionBaseSpec, ? extends RunBaseSpec, ? extends Runnable> getRuntime(
    String runtime
  ) {
    Runtime<? extends FunctionBaseSpec, ? extends RunBaseSpec, ? extends Runnable> concreteRuntime =
