...
public class RunnerFactory {
    ...
    public RunnerFactory(List<? extends Runner> runners) {
        runnerMap = runners.stream().collect(Collectors.toMap(this::getRunnerFromAnnotation, Function.identity()));
    }
    ...
    private String getRunnerFromAnnotation(Runner runner) {
        ...
        if (runnerClass.isAnnotationPresent(RunnerComponent.class)) {
            RunnerComponent annotation = runnerClass.getAnnotation(RunnerComponent.class);
            return annotation.runtime() + "+" + annotation.task();
        }
        throw new IllegalArgumentException("No @RunnerComponent annotation found for runner: " + runnerClass.getName());
    }
    ...
    public <R extends Runner> R getRunner(String runtime, String task) {
        ...
        if (concreteRunner == null) {
            throw new IllegalArgumentException("No runner found for name: " + runtime + "+" + task);
        }
