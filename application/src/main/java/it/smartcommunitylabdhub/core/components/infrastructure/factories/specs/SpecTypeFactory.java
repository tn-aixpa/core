...
public class SpecTypeFactory {
      ...
      for (BeanDefinition beanDefinition : candidateComponents) {
        ...
        try {
          ...
          Class<? extends Spec> specClass =
            (Class<? extends Spec>) Class.forName(className);
          SpecType specTypeAnnotation = specClass.getAnnotation(SpecType.class);
          String specKey =
            specTypeAnnotation.kind() +
            "_" +
            specTypeAnnotation.entity().name().toLowerCase();
  ...
  private List<String> getBasePackages() {
    ...
    ComponentScan componentScan =
      CoreApplication.class.getAnnotation(ComponentScan.class);
    if (componentScan != null) {
      Collections.addAll(basePackages, componentScan.basePackages());
    }
    if (basePackages.isEmpty()) {
      throw new IllegalArgumentException(
        "Base package not specified in @ComponentScan"
      );
    }
    return basePackages;
  }
