...
public class RunEntityBuilder {
  ...
  public RunEntity build(Run runDTO) {
    ...
    // Retrieve base spec
    RunBaseSpec spec = JacksonMapper.CUSTOM_OBJECT_MAPPER.convertValue(
      runDTO.getSpec(),
      RunBaseSpec.class
    );
    ...
    return EntityFactory.combine(
      ...
      builder ->
        builder
          ...
          .with(r ->
            r.setMetadata(
              ConversionUtils.convert(runDTO.getMetadata(), "metadata")
            )
          )
          .with(r ->
            r.setExtra(ConversionUtils.convert(runDTO.getExtra(), "cbor"))
          )
          .with(r -> r.setSpec(ConversionUtils.convert(spec.toMap(), "cbor")))
          .with(r ->
            r.setStatus(ConversionUtils.convert(runDTO.getStatus(), "cbor"))
          )
          .withIf(
