...
public class TaskEntityBuilder {
  ...
  public TaskEntity build(Task taskDTO) {
    ...
    // Retrieve base spec
    TaskBaseSpec spec = JacksonMapper.CUSTOM_OBJECT_MAPPER.convertValue(
      taskDTO.getSpec(),
      TaskBaseSpec.class
    );
        ...
        builder
          ...
          .with(r ->
            r.setFunction(
              Optional
                .ofNullable(
                  StringUtils.hasText(spec.getFunction())
                    ? spec.getFunction()
                    : null
                )
                .orElseThrow(() ->
          ...
          .with(t ->
            t.setMetadata(
              ConversionUtils.convert(taskDTO.getMetadata(), "metadata")
            )
          )
          .with(t ->
            t.setExtra(ConversionUtils.convert(taskDTO.getExtra(), "cbor"))
          )
          .with(t -> t.setSpec(ConversionUtils.convert(spec.toMap(), "cbor")))
          .with(t ->
            t.setStatus(ConversionUtils.convert(taskDTO.getStatus(), "cbor"))
          )
          .withIf(
