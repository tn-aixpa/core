...
public class WorkflowEntityBuilder {
    ...
    return EntityFactory.combine(
      ...
      builder ->
        builder
          ...
          .with(w ->
            w.setMetadata(
              ConversionUtils.convert(workflowDTO.getMetadata(), "metadata")
            )
          )
          .with(w ->
            w.setExtra(ConversionUtils.convert(workflowDTO.getExtra(), "cbor"))
          )
          .with(w -> w.setSpec(ConversionUtils.convert(spec.toMap(), "cbor")))
          .with(w ->
            w.setStatus(
              ConversionUtils.convert(workflowDTO.getStatus(), "cbor")
            )
          )
   ...
   */
  private WorkflowEntity doUpdate(
    WorkflowEntity workflow,
    WorkflowEntity newWorkflow
  ) {
    return EntityFactory.combine(
      workflow,
      newWorkflow,
