...
public class ContainerDeployRunner implements Runner {
    ...
    public Runnable produce(Run runDTO) {
        ...
        if (runAccessor.getTask().equals("deploy")) {
        ...
        } else if (runAccessor.getTask().equals("job")) {
            return Optional.of(runDTO)
                    .map(r -> this.validateJobRunDTO(r, runAccessor))
                    .orElseThrow(() -> new IllegalArgumentException("Invalid runDTO"));
        } else {
            throw new IllegalArgumentException("Invalid Task type for container, you should use (job, deploy)");
        }
    }
